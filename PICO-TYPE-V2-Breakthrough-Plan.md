...
*External-audit DONE 2026-09-23 (implements the amendment above): compliance check — existing Risk++ evals already external (generator/ToxicChat labels) and no FT ever used verifier-rejected tails; the missing piece was a dedicated hand-labeled sample, now shipped: `data/arth_audit_external.json` (69 hand-written items × 14 labels = 966 decisions; never trained, never fit thresholds/temps; cred-guarded) + `scripts/arth_audit_delivered.py`. First COMPLIANT delivered-error on shipped ft3 (`checkpoints_arth_ft3/arth_final.pt`): **flag-level 0.0455** (item exact-match 37/69); FN-dominant on rare labels (pos n=1–6, hand-phrased OOD — api_key recall 0.167, jwt/ssh/password/email/phone/pii/sql 0.000 at tiny n; jailbreak 1.000 but 9 FPs on inject/jail near-misses; secrets_aws 1.000). Honest read: in-head eval recall 0.74–1.00 overstates true generalization on fresh hand phrasing — exactly the Cheap-Verifiers blind spot; any paper/README delivered-error claim must cite this audit (numbers re-computable only via the JSON, never through the model itself). New `test_external_audit` (structure + creds + zero train overlap). **38/38 tests, ruff clean.** Next steps (amendment-aware): every delivered-error citation = external audit only; grow audit toward n≥200 with more positives per label before release claims; remaining Wk5+ items unchanged (hypercube deferred, merge arth→main is owner's call, Enron/AG/SST pre-registered rows need data+head decisions — Enron never entered V2 training data as shipped).*

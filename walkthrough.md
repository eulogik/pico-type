...
- **External hand-labeled audit (2026-09-23, Cheap-Verifiers arXiv:2609.01345 amendment)**: plan §3/§6 now require every Risk++ delivered-error number to come from an external hand-labeled sample, never from the model grading itself. Built `data/arth_audit_external.json` (**69 hand-written items × 14 labels = 966 decisions**, never trained/fit, cred-guarded) + `scripts/arth_audit_delivered.py`. First compliant delivered-error on shipped ft3: **0.0455 flag-level** (FN-dominant: rare-label positives 1–6 each with hand-phrased OOD positives — api_key recall 0.167, jailbreak 1.000 w/ 9 FPs on inject/jail near-miss confusion; item exact-match 37/69 = 0.536). Compliance check: existing evals already external (generator/ToxicChat labels), no FT ever used verifier-rejected tails — gap was only the hand-labeled sample, now closed. New `test_external_audit` guards structure/creds/zero-overlap. **38/38 tests, ruff clean.**

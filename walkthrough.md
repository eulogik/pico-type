...
- **External hand-labeled audit (2026-09-23, Cheap-Verifiers arXiv:2609.01345 amendment)**: plan §3/§6 now require every Risk++ delivered-error number to come from an external hand-labeled sample, never from the model grading itself. Built `data/arth_audit_external.json` (**211 hand-written items × 14 labels = 2954 decisions, ≥8 positives/label; never trained/fit, cred-guarded**) + `scripts/arth_audit_delivered.py`. Compliant delivered-error on shipped ft3: **0.0589 flag-level** (item exact-match 85/211 = 0.403) — FN-dominant recall collapse on hand-phrased OOD positives (0.000–0.500 across labels; jailbreak recall 0.500 but 32 FPs; secrets_github spec 0.921), versus in-head eval recall 0.74–1.00: **the Cheap-Verifiers blind spot measured on our own guardrail** (first 69-item draft read 0.0455 — larger/n-balanced audit moved it up, honest number stands). Compliance check: existing evals already external (generator/ToxicChat labels), no FT ever used verifier-rejected tails — gap was the hand-labeled sample, now closed. New `test_external_audit` guards structure/creds/zero-overlap/n≥200/pos≥100. **38/38 tests, ruff clean.**
- **AG/SST-2/Enron round — all three gates FAIL, shipped ckpt unchanged (2026-09-23)**: manifest amendment #3 (8k/6k/6k items, PII-masked caches, 51 Enron rows dropped by cred guard, held-out eval sets deduped vs train, all original roots byte-locked, `verify_manifest` PASS). Two bugs caught by deep-verify: **cal-loss explosion** (2-option rows batched with 4-option put -1e9 pads inside the calibrator log-softmax: cal 1.4e8 at step 4850 — killed before any ckpt write, fixed by slicing cal to real k, regression test added) and **pad-length non-invariance** (trunk pooled differs by max-abs 24.09 between len-pad and 1024-pad for identical input — gates script now uses the established fixed-1024 protocol). **Gates on shipped ft3: AG 0.2695 / SST 0.5023 / Enron 0.5315 vs 0.92/0.84/0.98 — FAIL, all at chance.** Ceiling probes prove the gap is representation-bound, not exposure-bound: AG held-out plateaus at 0.356 across epochs 3-8 while train loss stays 1.2-1.5; Enron reaches 0.791 @7.5 epochs with train 0.807 (train≈held-out). In-domain choice intact (0.8375 held-out synth). The 800-step round (ft4) learned none of the three tables → **discarded**, ft3@4799 remains shipped. **40/40 tests, ruff clean.**
- **Full verification pass — 17/17 gates (2026-09-23, ledger `GATES.md` untracked)**: every shipped number re-measured, not trusted: battery (60/60, 0.737, 0.929, 5.4ms, 2/11, 7/7), ToxicChat (0.846/0.970 PASS), audit (n=211, 0.0589), gate misses (0.2695/0.5023/0.5315), manifest PASS, ONNX 11.66MB IR8 + re-export VERIFY PASS, calibration ECE ≤0.10, thresholds refit byte-identical, ckpt strict-load step 4799 / 1 group / **all 89 trunk tensors byte-identical** (parity guarantee now proven). Negative controls: cal regression test fails on the reintroduced bug and passes after byte-exact restore; credential guards drop real-shaped creds and mask PII; purity overlaps all zero; docs carry canonical numbers with no stale/false claims; no unallowlisted creds in tracked files; CI runs ruff+pytest. **No project defect found.**
- **Wk5+(a) separate-tier variant — CLOSED with evidence (2026-09-23, branch `arth-sem-tier`)**: A/B probe first (`scripts/arth_probe_unfreeze.py`): unfreezing the trunk (lr 1e-4) lifts AG held-out 0.286→**0.660** where the frozen reference flatlines at 0.356 at identical exposure (trunk Δ 0.040) — the freeze, not the architecture, was the ceiling. Then the real variant (`checkpoints_arth_sem/arth_step_6000.pt`, resume-from-ft3 + immediate unfreeze, 1200 unfrozen steps at the 2h budget mark): **gates still all FAIL (AG 0.3035 / SST 0.4908 / Enron 0.6264)** — mixed objectives diluted the focused signal (~1.2 AG epochs vs 9.6 in the probe). Costs: parity 60/60→**54/60**, noul 0.929→0.823; gains: choice 0.737→**0.835**, FP 2/11→1/11; audit unchanged 0.0589; latency 4.9ms. Nothing shipped changed. Only remaining Wk5+ item: hypercube 8k.
- **Audit metric correction + root-cause diagnosis (2026-09-23, after an "are you sure?")**: flag-level delivered error is **negative-dominated** — an all-negative predictor scores 0.0379 vs our 0.0589, so the aggregate rewards not flagging (the Cheap-Verifiers trap, inside the audit script itself). Script now leads with **positive recall 12/112 = 0.1071 + balanced error 0.4594**; flag-level error is secondary next to its baseline. Recall-collapse root causes measured: (a) operating points — spec≥0.97 Youden fit produced 0.93–1.00 thresholds (pii_card 1.00 never fires) and **5/14 labels have no fitted threshold**, though ranking is intact (AUC 0.68–0.95) and thr 0.01 recovers 0.50–0.75 recall on four labels; (b) real score collapse on password/email/phone/jwt/api_key (p≈0.000 — no threshold helps; training-data gap); (c) pii_ssn AUC 0.42 (broken beyond thresholds). Revised order: report fix → threshold refit (owner approval, never fit on audit) → generator diversity for collapsed labels → hypercube.
- **Threshold refit shipped (2026-09-23, owner picked "balanced 3/11")**: Youden-J provably wrong for this head (with saturated in-dist positives it pushes T onto the hardest negative → OOD-dead 0.93–1.00 thresholds). New policy: lowest T with fit-spec ≥0.97 (+0.90 inject) + documented battery-probe floors (sql 0.0311 / xss 0.0261 / jb 0.398); audit never fit on. Results: fit/eval recall 1.00, spec ≥0.95; battery FP **3/11** (2 old inject + accepted SELECT→sql), demos 7/7, parity 60/60; ToxicChat **0.857/0.970 PASS** (was 0.846); external audit **positive recall 0.107→0.223 (doubled)**, balanced error 0.459→0.408 — pii_card 0→0.75, sql 0→0.38, xss 0.13→0.50. Remaining zero-recall = training-data causes (5 never-trained labels, api_key/email collapse, pii_ssn AUC 0.42, inject probe-blocked) → generator-diversity round is next, ahead of hypercube. ruff + 40/40. Incident: Mac watchdog reboot mid-eval (no panic; possible load contribution) → standing rule: ask before heavy compute; light evals use nice+2 threads.
- **Generator-diversity data layer — manifest amendment #4 (2026-09-23, `arth-sem-tier`)**: closes the training-data root cause of the audit gap. 6 new multi-frame generators (`gen_api_key` with sk-/sk_live-/Bearer/ApiKey forms, `gen_jwt`, `gen_ssh_key`, `gen_password`, `gen_email`, `gen_phone`; 5–9 pos + 5–6 neg frames each, 43–49 distinct/50) + `gen_pii_ssn` rewritten 8/5 frames (single-vault-frame keying = the AUC 0.42 cause; phone-field neg removed to avoid channel conflict). **Bug caught:** `riskpp_synth` builder hardcoded `zip(..., range(8))` — would have silently dropped all 6 new gens from training. Proof: manifest diff = only riskpp_synth changed (4000→7000, root 757df8ac→e61371c8), all other roots byte-locked, `verify_manifest` PASS. New `test_riskpp_label_coverage` regression-closes the original never-trained-labels blind spot; audit purity 5 seeds × 5600 rows = 0 overlaps; cred guard 0 failures. Thresholds intentionally NOT refit until after training (14-label refit + audit reveal come post-FT). ruff + **41/41**. Next: owner-approved training round (frozen-trunk risk-focused FT from ft3), then refit/battery/ToxicChat/audit verification, then hypercube.
- **FT5 round + composite candidate (2026-09-25, owner-approved 800 steps)**: resume ft3→step 5600, trunk frozen (`--freeze-steps 99999`, hash `47efcfff` unchanged), ~2s/step. ft5-as-is drifted semantic heads (AG 0.2695→0.2455, Enron 0.5315→0.4902; SST 0.5023→0.5344) — **true ft3 baseline gates now on record: 0.2695/0.5023/0.5315 all FAIL** (the earlier 0.3035/0.4908/0.6264 were the variant's unfrozen numbers — record corrected). Since head families are independent (trunk 0/89 changed, riskpp 2/8, semantic 30/30, relational 2/2), shipped candidate = **`checkpoints_arth_ft5/arth_final_composite.pt` (ft3 + ft5 `riskpp.*` only)**, integrity-proven and gate-identical to ft3. Composite ledger: parity **60/60**, choice 0.737, noul 0.929, latency 6.6ms, demos 7/7, **FP 3/11** (owner choice B: sql floor 0.150 keeps `print('hello world')` accepted, preserves audit sql 2/8), ToxicChat **0.824/0.973 PASS|PASS**. **Audit: positive recall 0.107→0.536 (60/112), balanced error 0.260** — jwt/ssh/email 8/8, password 0.875, phone 0.750, api_key 0.625, ssn 0.125; collateral aws 0.375→0, card 0.75→0.5, xss 0.5→0.375, spec 0.944. All 14 thresholds fitted (test contract 9→14). ruff + 41/41. Open: owner decision on ONNX re-export/HF redeploy of the composite, then hypercube.
- **Local ONNX export from composite (2026-09-25, no deploy)**: `arth_export_onnx.py --ckpt …composite.pt` → `checkpoints/arth_full_base.onnx` 11.66MB IR8 **VERIFY PASS** (err 1.144e-05, parity 60/60, semantic 300/300, risk flags 11/11 under 14-label thresholds). **INT8 4.28MB semantic decision parity slipped 300/300 → 298/300** (parity 60/60 + flags 11/11 still OK; risk-head change shifted quant rounding) — below the ship bar; FP32 canonical. Gotcha: `verify()` on an INT8 path re-quantizes into a `*_int8_int8.onnx` stray. HF/Space/CF untouched (still ft3 + old client thresholds) — deploy round awaits owner. ruff + 41/41, commit `42e130a`.
- **Hypercube 8k wiring — built, measured, CLOSED with evidence (2026-09-25)**: wiring per arXiv:2609.18145 in `arch.py` (`hypercube_sparse_attention`: query i sees only {self, i XOR 2**bit}; manual 2-way softmax = exact match to a masked-dense reference; O(L) memory, no L×L mask) + `ArthModel.pooled(wiring='hypercube')` with weight-tied blocks over rotated dimensions (default virtual count = log₂n; in-place = 2). Opt-in, inference-only, dense path byte-identical; `m.wiring` runs the whole existing battery under either mode. Tests reproduce the paper's claims: rotated log₂n dims reach every position, a fixed pattern does not (5 new, **46/46**, ruff clean). **Gates: latency@1k in-place 7.3ms ≤ dense 10.4ms PASS, full-rotation 25ms FAIL; @8k 267.6 ≤ 314ms PASS (1.17×, not 2.4× — conv/MLP-dominated 2-block trunk); quality FAILS — choice 0.737→0.46-0.47, relational 24/30→2-7/30, ToxicChat recall 0.824→0.000 (AUROC 0.456, inverted); ECE ≤0.10 holds (calibration fine, ranking dead); 8k localized-signal demos 0.000 under BOTH wirings (dense@8k control proves pooling dilution, not wiring). **Conclusion: the paper's 1/32-links result is a TRAINING result — inference-only wiring on a dense-trained frozen trunk is unusable; the plan's "no training overhead" conflated wiring cost with training cost. A hypercube-wired variant would need a fresh training-approval item; threshold-nudging to mask the collapse is rejected as the Cheap-Verifiers trap.** Nothing shipped changed. Wk5+ queue now empty. Commit `7fc9d08`.
...
- **Merge + deploy (2026-09-26)**: `arth-sem-tier` ff-only → main (`c5ef88d`, 46/46). Deep-verify corrected the deploy premise: ARTH had **never** been shipped (HF repos/Space served only v02 tiers, `picotype.dev` worker dead). Phase A: 5 composite artifacts (FP32+INT8 ONNX, 14-label thresholds, temps, composite ckpt) uploaded to `eulogik/pico-type`, sha256 read-back PASS. Phase B: `gradio_app.py` = ARTH default + **Risk++ (14)** tab with `[FLAG]` markers; caught a ship-blocker in cross-check (thresholds JSON key order ≠ `RISK_PLUS_LABELS` → position-zip mislabeled flags; fixed with explicit label list + set assertion); 11-demo local torch cross-check PASS; pushed to Space, rebuilt → RUNNING (old quota blocker didn't bite), live API **4/4 PASS** (AWS flags match torch to 4dp; benign shows only known phone FP; legacy tiers intact). Phase C: worker documented dormant (unreachable), extension local-only. **Live: huggingface.co/spaces/eulogik/pico-type now serves the composite + 14-label thresholds.**
- **Docs/SEO round (2026-09-26)**: repo `README.md` + `MODEL_CARD.md` now carry ARTH V2 (metric table, 14 labels, ONNX + Space-API quickstarts, measured limitations). HF model card same + discovery frontmatter (6 new tags, `model-index` with audit + ToxicChat blocks — YAML re-validated after upload). Space README replaced placeholder with real usage/metrics/limits; rebuild → RUNNING, live smoke PASS. Story is now discoverable and honest.

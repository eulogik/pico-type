...
We use AdamW optimizer with $\beta_1 = 0.9$, $\beta_2 = 0.999$, weight decay 0.01, and a linear warmup of 50 steps followed by cosine decay to 0. Gradient clipping at 1.0 is applied. Training uses mixed precision (bfloat16 on CUDA, float32 otherwise) with batch size 32.
...
subtype & 24 & 95.4\% & 285 \\
code\_lang & 62 & 47.1\% & 85 \\
text\_lang & 30 & 81.3\% & 91 \\
file\_mime & 90 & 100.0\% & 250 \\
risk (mAP) & 6 & 90.5\% & -- \\
\midrule
\multicolumn{3}{l}{Inference time (CPU)} & 11.2 ms \\
\multicolumn{3}{l}{Model size (base tier ONNX)} & 206 KB \\
\multicolumn{3}{l}{Total parameters (base)} & 1.48M \\
\bottomrule
\end{tabular}
\caption{\label{tab:results} Evaluation results on 1000 synthetic samples.}
\end{table}

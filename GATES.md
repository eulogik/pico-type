...
- [x] FT5 COMPOSITE VERIFICATION (2026-09-25, hand-run; verify_all.py lost in the watchdog reboot so no auto-evidence)
  CANDIDATE: checkpoints_arth_ft5/arth_final_composite.pt (= ft3 student + ft5 riskpp.* only)
  parity 60/60 | choice 0.737 | noul 0.929 | latency 6.6ms | demos 7/7 | FP 3/11 (owner choice B)
  ToxicChat 0.824/0.973 PASS|PASS (gates 0.78/0.93)
  audit eval-only: positive recall 0.536 (60/112), balanced error 0.260, spec 0.944
  semantic gates: AG 0.2695 / SST 0.5023 / Enron 0.5315 = byte-identical to shipped ft3 (all FAIL, pre-existing)
  ruff clean; pytest 41/41

- [x] LOCAL ONNX EXPORT (2026-09-25, composite; NOT deployed)
  checkpoints/arth_full_base.onnx 11.66MB IR8 VERIFY PASS (err 1.144e-05, parity 60/60, semantic 300/300, flags 11/11, 14-label thresholds)
  INT8 4.28MB: parity 60/60, flags 11/11, semantic argmax 298/300 -> below the 300/300 ship bar; FP32 canonical
  ruff clean; pytest 41/41; no HF/CF/Space upload performed

- [x] HYPERCUBE 8k WIRING GATES (2026-09-25, hand-run; scripts/arth_eval_hypercube.py)
  structure: exact vs masked-dense ref (batched+padding), log2n reachability, fixed-pattern fails, dense path byte-identical | 5 new tests, 46/46, ruff clean
  dense regression: parity 60/60, choice 0.737, noul 0.929, demos 7/7, FP 3/11 (unchanged)
  latency@1k: in-place 7.3ms <= dense 10.4ms PASS | full-rotation 25.0ms FAIL | @8k: 267.6 <= 314.0 PASS (1.17x, not 2.4x)
  quality@1k: choice 0.471/0.457 (dense 0.737) FAIL | relational 7/30, 2/30 (dense 24/30) FAIL
  ToxicChat: recall@T 0.000/0.000 (dense 0.824) FAIL | AUROC 0.711/0.456 FAIL
  ECE choice: 0.057/0.060 <=0.10 PASS (dense 0.044)
  8k demos: 0.000 under hypercube AND 0.000 under dense@8k control -> pooling dilution, not wiring
  VERDICT: quality gates FAIL — inference-only wiring on a dense-trained trunk; paper's result requires training with the wiring

- [x] MERGE + DEPLOY (2026-09-26, owner-approved)
  merge: arth-sem-tier ff-only -> main c5ef88d, pushed, ruff + 46/46 post-merge
  premise correction: ARTH was never deployed (repos/Space = v02 tiers only; picotype.dev dead) — first ship, not refresh
  model repo: 5 artifacts uploaded to eulogik/pico-type root, sha256 read-back PASS x5
  Space: gradio_app.py ARTH default + Risk++ tab; label-order ship-blocker caught+fixed (JSON order != RISK_PLUS_LABELS); 11-demo local torch cross-check PASS; app.py sha PASS
  live: Space RUNNING (cpu-basic), gradio_client 4/4 PASS (flags == local torch 4dp; benign known-FP; legacy tier OK)
  worker: picotype.dev unreachable -> dormant documented | extension: local-dev unchanged
